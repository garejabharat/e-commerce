...
const CartItem: React.FC<CartItemProps> = ({
  ...
  const removeItemHandler = () => {
    ...
    enqueueSnackbar(`Successfully removed ${title.substring(0, 55)}... been Removed from your cart`, {
      variant: "success",
    });
  };
  const decreseCart = () => {
    dispatch(decrementCartItem(id));
    enqueueSnackbar(`You've changed ${title.substring(0, 55)}... QUANTITY to '${quantity - 1}'`, { variant: "success",
  });
  };
  const cart = {
    id: id,
  ...
  const handleAddToSaveForLater = () => {
    ...
    enqueueSnackbar(`You've Added ${title.substring(0, 55)}... with Quantity ${quantity} for the Saved Later!`, { variant: "success" });
  };
